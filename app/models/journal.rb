class Journal < ActiveRecord::Base
  ...
  def self.report_ledger_balance(periods, company, unit, project,car, show_only_active_accounts,show_last_period,journal_type)
    ...
    #calculate each of the column with DbStoredProcedure report_balance
      ...
      balance = DbStoredProcedure.fetch_db_records("report_balance(false, #{show_last_period},#{show_only_active_accounts},#{company_id},#{car_id},#{unit_id},#{project_id},#{journal_type_id},#{_balance},#{_balance_previous},#{_balance_last},#{_balance_last_previous})")
      #Result Accounts report part
      result = DbStoredProcedure.fetch_db_records("report_balance(true, #{show_last_period},#{show_only_active_accounts},#{company_id},#{car_id},#{unit_id},#{project_id},#{journal_type_id},#{_result},#{_result_previous},#{_result_last},#{_result_last_previous})")
    ...
    balance.each do |row|
      case row["acc_number"]
      when 1000..1999 then total_accounts["1000"] += row["acc_nb"]
      when 2000..2999 then total_accounts["2000"] += row["acc_nb"]
      end
    end

    result.each do |row|
      case row["acc_number"]
        when 3000..3999 then total_accounts["3000"] += row["acc_nb"]
        when 4000..4999 then total_accounts["4000"] += row["acc_nb"]
        when 5000..5999 then total_accounts["5000"] += row["acc_nb"]
        when 6000..6999 then total_accounts["6000"] += row["acc_nb"]
        when 7000..7999 then total_accounts["7000"] += row["acc_nb"]
        when 8000..8999 then total_accounts["8000"] += row["acc_nb"]
        when 9000..9999 then total_accounts["9000"] += row["acc_nb"]
      end
    end
